...
def _leaky_clamp_straight_through(x, leak=0.01):
    """clamp(x, 0, 1), but with a small leak-slope gradient outside [0, 1] instead of
    the hard clamp's exact zero -- a straight-through estimator: the forward value is
    bit-for-bit the plain hard clamp (`hard`, detached, so it contributes no gradient
    of its own), and `soft - soft.detach()` is exactly 0 in value but carries `soft`'s
    gradient backward. Confirmed necessary empirically, not just in theory: on a real
    trained rollout, the reflex's correction term sat exactly at 0 or 1 on 95.3% of all
    steps (see LearnableForagingReflex), so the plain hard clamp gave angle_gain/
    rate_gain a real gradient on well under 5% of steps -- consistent with them moving
    by under 0.5% over 20k training steps. `soft` matches the hard clamp exactly inside
    [0, 1] (unchanged gradient there), and only differs in the saturated tails, where
    it leaks a small (`leak`-scaled) slope instead of none.
    """
    hard = x.clamp(min=0, max=1).detach()
    soft = torch.where(x < 0, leak * x, torch.where(x > 1, 1 + leak * (x - 1), x))
