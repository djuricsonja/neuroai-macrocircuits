...
from macrocircuits.controllers import check_controller, make_controller
...
class NCAPSwimmerPolicy(nn.Module):
    ...
    `controller` is an optional steering controller (see `controllers`), which reads the
    task's egocentric to_target / to_obstacle vector out of the same observation and
    drives the circuit's turn inputs with it -- the ES counterpart of what
    `ncap.SwimmerActor` does for the RL path. A fixed reflex adds no parameters, so only
    the circuit's own `bneuron_turn` weight is evolved; an `MLPController` is an
    nn.Module, so assigning it here registers it and ES evolves its weights too --
    with the same `sigma` as the circuit's, over a far larger parameter vector.

    The vector it reads is in raw task units on both paths -- tonic normalizes inside
    its observation encoders, which `SwimmerActor` does not use -- so a controller tuned
    on one (e.g. `make_obstacle_avoidance_reflex`'s `reaction_distance`) behaves the
    same on the other.
    """
...
def run_es(
    ...
    `controller` plugs a steering controller into NCAP on the target/obstacle tasks --
    a fixed reflex or a learned MLP, exactly as on the RL path; see `NCAPSwimmerPolicy`
    and `controllers.CONTROLLERS`. Without one the circuit swims unsteered there, so
    only its reward changes and not its behaviour.
    """
